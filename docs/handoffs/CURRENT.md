...
主路线图已经改为“中文版儿童控制商业版优先”，英文实装后置。第一阶段合规底稿已经列出数据清单、权限说明、家长同意流程、平台能力边界和审核风险。
...
1. 提交并推送本轮文档。
2. 用真机应用商店核验儿童控制类竞品，尤其是华为、小米、OPPO、vivo。
3. 依据合规底稿和竞品分析，确定 Android 儿童控制核心版的第一版功能范围。
4. 进入账号、支付、后台管理前，先评估 ShipAny 模板、Sa-Token、Shiro 等可复用资源。
5. 不急着写最终隐私政策、用户协议或应用商店数据安全表，等核心功能和第三方服务稳定后再定稿。
6. 继续维护现有 PC + Android 统计版稳定性，但不要把 Windows 做成儿童控制主平台。
